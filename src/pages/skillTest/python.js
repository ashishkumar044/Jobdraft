...
import {Python} from '../../util/skillTestData/coding'
...
class python extends Component {
  ...
  loadTest = () => {
    ...
    this.setState(() => {
        return {
            questions: Python[currentQuestion].question,
            options: Python[currentQuestion].options,
            answers: Python[currentQuestion].answer
        }
    })
}
...
handleFinish = () => {
  const {userAnswer, answers, score} = this.state
  // if(this.state.currentQuestion === Python.length - 1){
      this.setState({
          endTest: true
      })
  // }
            ...
            <Grid container>
              ...
              <Grid item sm={10} xs={12}>
                <Card elevation={1} className={classes.scoreCard}>
                  <CardContent>
                    <h4 variant="h4" className={classes.header}>Score Card </h4>
                      <form onSubmit={this.handleSubmit}>
                      <h3>Well Done!! You scored {this.state.score}.</h3>
                            <Card elevation={0} className={classes.buttonCard}>
                                <CardContent>
                                    <Button
                                        type="submit"
                                        variant="contained"
                ...
                <Card elevation={1} className={classes.card}>
                  ...
                  <CardContent>
                      <Typography variant="body1" className={classes.questionCount}>
                        {`Questions ${currentQuestion + 1} of ${Python.length}`}
                      </Typography>
                        <h3>{questions}</h3>
                      {options.map(option => (
                        ...
                        <input 
                            ...
                            onClick={() => {
                              // this.setState((prevState) => {return{checkedRadio: !prevState.checkedRadio}})
                      ...
                      {currentQuestion < Python.length - 1 &&
                          <Fragment>
                              <Button 
                                color="primary" 
                                variant="contained" 
...
export default connect(mapStateToProps, { postProject })(withStyles(styles)(python))